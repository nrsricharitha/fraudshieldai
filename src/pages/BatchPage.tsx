...
import { UploadCloud, Download, CheckCircle2, AlertTriangle, FileText, ArrowRight } from 'lucide-react';
...
export function BatchPage() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      addToast({
        ...
        message: `Successfully processed ${data.total_processed} transactions. Detected ${data.fraud_detected} frauds.`,
      });
    } catch (err: any) {
      addToast({
        type: 'error',
  ...
  const handleDownloadSample = async () => {
    try {
      ...
      if (!samples || samples.length === 0) {
        addToast({ type: 'warning', title: 'No sample data available' });
        return;
      }
      ...
      link.setAttribute('download', 'sample_test_transactions.csv');
  ...
  const handleExportResults = () => {
    ...
    const link = document.createElement('a');
    link.setAttribute('href', encodedUri);
    link.setAttribute('download', `scored_transactions_${Date.now()}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">Batch Transaction Analysis</h1>
          <p className="text-sm text-slate-500 mt-1">
            Upload and score large CSV files through the real trained XGBoost & Isolation Forest pipelines.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <Card>
        <CardHeader
          ...
          subtitle="Supports standard Kaggle Credit Card Fraud format (Time, Amount, V1 to V28)"
          icon={<UploadCloud className="w-4 h-4" />}
        />
        <div className="p-6 border-2 border-dashed border-white/[0.08] hover:border-brand-500/40 rounded-xl transition-colors text-center bg-ink-900/40">
          <input
            type="file"
            accept=".csv"
            id="csv-file-input"
          ...
          <label htmlFor="csv-file-input" className="cursor-pointer block">
            ...
            <p className="text-xs text-slate-500 mt-1">
              File must contain Time, Amount, and PCA numerical columns V1 through V28.
            </p>
          </label>
          ...
          {file && (
            <div className="mt-4 flex items-center justify-center gap-3">
              ...
              <Button
                ...
                disabled={loading}
              >
                {loading ? 'Executing Real ML Scoring...' : 'Run Real ML Batch Scoring'}
              </Button>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6 animate-fade-in">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="p-4 rounded-xl bg-red-500/10 border border-red-500/20">
              <p className="text-xs text-red-400 uppercase tracking-wider">Frauds Detected</p>
            ...
            <div className="p-4 rounded-xl bg-cyan-500/10 border border-cyan-500/20">
              <p className="text-xs text-cyan-400 uppercase tracking-wider">Anomalies Flagged</p>
            ...
            <div className="flex items-center justify-between p-4 border-b border-white/[0.06]">
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Decision threshold applied: <span className="font-mono text-amber-400">{batchResult.threshold_used}</span>
                </p>
              </div>
              <Button
                variant="outline"
                  ...
                  {batchResult.results?.map((row: any, i: number) => (
                    <tr key={i} className="hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-3">
                        <PredictionBadge
                          prediction={row.prediction === 1 ? 'Fraudulent' : 'Legitimate'}
                        />
                      </td>
